...
ActionButtonAndStatus::ActionButtonAndStatus(MIDIEnviromentTestType testType, MIDIEnviromentTestOperationType operationType) : ActionButtonAndStatus()
{
    testID.testType = testType;
    testID.operationType = operationType;
    testID.midiChannel = 1;

    ProcessorData *pData = ProcessorData::getInstance();
    pData->midiEnviromentTestManager.addSubscriberToTestChange(testID, this);
}
//[/MiscUserDefs]
...
ActionButtonAndStatus::ActionButtonAndStatus ()
{
    ...
    actionButton.reset (new TextButton ("new button"));
...
//==============================================================================
void ActionButtonAndStatus::paint (Graphics& g)
{
...
void ActionButtonAndStatus::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    if (buttonThatWasClicked == actionButton.get())
    {
        ...
        ProcessorData *pData = ProcessorData::getInstance();
        MIDIEnviromentTestManager *manager = &pData->midiEnviromentTestManager;
        
        const void * address = static_cast<const void*>(&pData->midiEnviromentTestManager.testEntryMap[testID].theTest.allUniqueMessages);
        std::stringstream ss;
        ss << address;
...
//    statusBar->setTooltip(TRANS(status));
    ...
    ProcessorData *pData = ProcessorData::getInstance();
    actionButton->setButtonText(std::to_string(pData->midiEnviromentTestManager.testEntryMap[testID].theTest.allUniqueMessages.size()) + "/" + std::to_string(pData->midiEnviromentTestManager.testEntryMap[testID].theTest.initalUniqueMessageCount));
    
    std::string test = "";
    if (status == TIMEOUT)
    {
        for (auto it = pData->midiEnviromentTestManager.testEntryMap[testID].theTest.allUniqueMessages.begin(); it != pData->midiEnviromentTestManager.testEntryMap[testID].theTest.allUniqueMessages.end(); ++it )
        {
...
<JUCER_COMPONENT documentType="Component" className="ActionButtonAndStatus" componentName=""
  ...
  <TEXTBUTTON name="new button" id="747d04c3852ec15e" memberName="actionButton"
              virtualName="" explicitFocusOrder="0" pos="0% 0% 50.044% 100%"
  ...
  <GENERICCOMPONENT name="new component" id="ddfbc579fc8ae26b" memberName="statusBar"
                    virtualName="" explicitFocusOrder="0" pos="50.044% 0% 50.044% 100%"
