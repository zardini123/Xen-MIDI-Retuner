...
// Reimplementation of __iscsymf
// https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/iscsym-functions?view=vs-2019
int iscsymf(int c) {
	return isdigit(c) || isalpha(c) || static_cast<char>(c) == '_';
}

bool EvalKeyAndValue(std::string & str, std::string & strKey, std::string & strValue)
{
	std::string::size_type	pos = str.find('=');
	if ( (pos == std::string::npos) || (!iscsymf(str.at(0))) )
		return false; // error: no '=' or first char of key is invalid

	std::string str2 = str.substr(0, pos);
	strKey = ToLower(Trim(str2));
